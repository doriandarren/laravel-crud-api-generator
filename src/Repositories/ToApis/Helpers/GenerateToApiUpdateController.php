...
class GenerateToApiUpdateController
{
                             ...
                             $relationType): bool
    {
        ...
        $contents .= HelperFiles::formatLineBreakAndTab('$data = $this->repository->update($'.$tableSingular.'->id, $request->all());',null,1,4);
        $contents .= HelperFiles::formatLineBreakAndTab('}else{',null,1,3);
        $contents .= HelperFiles::formatLineBreakAndTab('return $this->respondWithError(\'Error updated\', [\'e\' => trans(\'validation.role_not_allowed\')]);',null,1,4);
        $contents .= HelperFiles::formatLineBreakAndTab('}',null,1,3);
        $contents .= HelperFiles::formatLineBreakAndTab('}',null,1,2);
        $contents .= HelperFiles::formatLineBreakAndTab('return $this->respondWithData(\'' . $classNameSingularUp . ' updated\', $data);',null,1,2);
        $contents .= HelperFiles::formatLineBreakAndTab('}',null,2,1);
