...
class GenerateToApiShowController
{
                             ...
                             $relationType, $path): bool
    {
        ...
        $contents .= HelperFiles::formatLineBreakAndTab('}elseif($this->isManager(auth()->user()->roles)){',null,2,2);
        $contents .= HelperFiles::formatLineBreakAndTab('$data = $this->repository->show' . EnumFolderToApi::AUTH_BY_MANAGER . '(auth()->user()->employee->company_id, $' . $tableSingular . '->id);',null,1,3);
        $contents .= HelperFiles::formatLineBreakAndTab('return $this->respondWithData(\'' . $classNameSingularUp . ' show\', $data);',null,2,3);
